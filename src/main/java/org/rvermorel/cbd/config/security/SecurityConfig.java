...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests().antMatchers("/", "/front","/images/**","/feeds","/rankings","/partners","/cbdfiles","/assos").permitAll().antMatchers("/back","/back/**","/**/add**","/**/delete**","/**/update**")
                .access("hasRole('ADMIN')").and().formLogin().loginPage("/cbd84/login").successHandler(customSuccessHandler).usernameParameter("username").passwordParameter("password").and().exceptionHandling()
